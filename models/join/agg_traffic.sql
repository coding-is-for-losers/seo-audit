...
ifnull(CASE WHEN sum(total_organic_transactions_30d) > 0 THEN sum(transactions_30d) 
	ELSE sum(goal_completions_all_goals_30d) END, 0) as blended_conversions_30d,
ifnull(CASE WHEN sum(total_organic_transactions_30d) > 0 AND sum(sessions_30d) > 0 THEN max(ecommerce_conversion_rate_30d)
	ELSE max(goal_conversion_rate_all_goals_30d) END, 0) as blended_conversion_rate_30d,
